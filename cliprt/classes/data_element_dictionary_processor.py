...
class DataElementDictionaryProcessor:
    ...
    def ded_is_configured(self):
        ...
        # indicator.
        col_idx = col_headings[self.settings.COL_HEADINGS[self.settings.DEST_WS_COL_NAME]]
        ...
        col_idx = col_headings[self.settings.COL_HEADINGS[self.settings.DE_FORMAT_COL_NAME]]
        ws_columns = self.ws.iter_cols(
            min_col=col_idx, 
            max_col=col_idx, 
            min_row=self.ws.min_row+1)
        de_column = list(ws_columns)[0]
        for de_cell in de_column:
            if not de_cell == None and self.settings.IDENTIFIER_DESIGNATION in de_cell.value:
    ...
    def hydrate_ded(self):
        ...
        col_idx = col_headings[self.settings.COL_HEADINGS[self.settings.DE_COL_NAME]]
        ws_columns = self.ws.iter_cols(
            min_col=col_idx, 
            max_col=col_idx, 
            min_row=self.ws.min_row+1)
    ...
    def parse_dest_de_format_str(self, de_name, dest_de_format_str):
        ...
        for dest_de_format in self.util_make_list(dest_de_format_str):
            ...
            if self.settings.IDENTIFIER_DESIGNATION in dest_de_format_list and self.settings.FRAGMENT_DESIGNATION in dest_de_format_list:
    ...
    def process_dest_ind(self, de_name, ws_dest_ind):
        ...
        if not ws_dest_ind in self.dest_ws_registry.dest_ws_by_ind_list:
            ...
            self.dest_ws_registry.add_ws(self.wb, ws_dest_ind)
    ...
    def read_de_config(self, col_headings, de_name, de_row_idx):
        ...
        # Dest Element
        # ------------
        col_idx = col_headings[self.settings.COL_HEADINGS[self.settings.DEST_DE_COL_NAME]]
