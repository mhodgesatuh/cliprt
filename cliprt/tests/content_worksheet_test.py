...
class ContentWorksheetTest:
    ...
    def build_etl_map_test(self):
        ...
        Unit test
        """
        content_ws_name = self.client_info.wb.sheetnames[2]
    ...
    def process_ws_rows_test(self):
        ...
        assert content_ws.ws.max_row == 20

        content_ws.build_etl_map()
        assert content_ws.process_ws_rows()
        assert len(self.client_info.wb.sheetnames) == 5
        assert 'comm_report_for_ims' in self.client_info.wb.sheetnames
        assert 'comm_report_for_fb' in self.client_info.wb.sheetnames
