...
while cliprting:
    ...
    if not client_info_wb.ded_is_configured():
        prompt_a = '\nYou need to open the Excel-compatible workbook and configure the DED for your reporting requirements.'
        prompt_b = 'Ready to configure your reports?'
        if user_requests(prompt_a, prompt_b):
            sys.exit(0)
        else:
            inputting = False
    ...
    if client_info_wb.ded_is_configured():
        ...
        sys.exit(0)
    elif client_info_wb.has_a_ded_ws():
        print('Please configure the DED.  Your reporting requirements are currently undefined.')
    else:
        print('You need to first create the DED and define your reporting requirements.')
